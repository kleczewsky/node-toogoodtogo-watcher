...
function getInterval(configPath, minimumIntervalInMs) {
  ...
  const configuredIntervalVariance = config.get('api.intervalVaryByPercent', 0);
  const varianceMax = configuredIntervalInMs*(configuredIntervalVariance/100)
  return _.isFinite(configuredIntervalInMs)
    ? Math.max(configuredIntervalInMs, minimumIntervalInMs) =* _.random(configuredIntervalInMs, configuredIntervalInMs + varianceMax)
    : minimumIntervalInMs;
}
